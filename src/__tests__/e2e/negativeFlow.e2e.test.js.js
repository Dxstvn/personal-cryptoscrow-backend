...
describe('E2E Negative Flow Tests', () => {
  ...
  beforeAll(async () => {
    ...
    if (response.status === 201 && response.body.user && response.body.user.uid) {
      ...
      const loginResponse = await apiClient.post('/auth/login', { email: userAEmail, password: userAPassword });
  ...
  afterAll(async () => {
    ...
    // await stopTestServer(); // stopTestServer is a no-op currently
  ...
  describe('Authentication Negative Flows', () => {
    test('Should fail to signup with an already existing email', async () => {
      ...
      const response = await unauthApiClient.post('/auth/signup', signupData);
    ...
    test('Should fail to signup with missing email', async () => {
      const response = await unauthApiClient.post('/auth/signup', { password: 'password123', walletAddress: '0x123' });
    ...
    test('Should fail to signup with missing password', async () => {
      const response = await unauthApiClient.post('/auth/signup', { email: 'newuser@example.com', walletAddress: '0x123' });
      expect(response.status).toBe(400);
      expect(response.body.error).toContain('Email and password are required');
    ...
    test('Should fail to login with non-existent email', async () => {
      const response = await unauthApiClient.post('/auth/login', { email: 'nosuchuser@example.com', password: 'password123' });
    ...
    test('Should fail to login with incorrect password', async () => {
      const response = await unauthApiClient.post('/auth/login', { email: userAEmail, password: 'wrongPassword' });
  ...
  describe('Contact Management Negative Flows', () => {
    ...
    beforeAll(async () => { // Nested beforeAll for this describe block
      ...
      let signupResponse = await unauthApiClient.post('/auth/signup', userBSignupData);
    ...
    test('Should fail to respond to a non-existent invitation', async () => {
      ...
      const loginResponseB = await unauthApiClient.post('/auth/login', { email: userBEmail, password: userBPassword });
  ...
  describe('Transaction Negative Flows', () => {
    ...
    // Transaction creation requires a valid contactId (UID of an accepted contact).
    // This setup is complex for negative flows without a guaranteed accepted contact.
    // We'll focus on input validation and auth for now.

    test('Should fail to create transaction without authentication', async () => {
      const response = await unauthApiClient.post('/transaction/create', { transactionName: 'test' });
      expect(response.status).toBe(401);
      console.log('✅ Test: Create transaction unauthenticated failed as expected.');
    });

    test('Should fail to create transaction with missing contactId', async () => {
      const transactionData = {
        // contactId is missing
        transactionType: 'simple_transfer',
        transactionName: 'Test Missing Contact ID',
        transactionAmount: '0.01',
        currency: 'ETH',
      };
      const response = await apiClient.post('/transaction/create', transactionData);
      expect(response.status).toBe(400); 
      expect(response.body.error).toMatch(/Contact ID is required|contactId must be a non-empty string/i);
      console.log('✅ Test: Create transaction with missing contactId failed as expected.');
    });
    
    test('Should fail to create transaction with non-existent contactId', async () => {
      const transactionData = {
        contactId: 'non-existent-contact-uid',
        transactionType: 'simple_transfer',
        transactionName: 'Test NonExistent Contact ID',
        transactionAmount: '0.01',
        currency: 'ETH',
      };
      const response = await apiClient.post('/transaction/create', transactionData);
      // This might be 404 (contact not found) or 400 (invalid reference)
      // depending on how deep the validation goes before checking contact existence.
      // Assuming contact existence is checked and results in an error related to the contact.
      expect([400, 404]).toContain(response.status);
      if (response.status === 404) {
        expect(response.body.error).toContain('Contact not found');
      } else { // 400
         expect(response.body.error).toBeDefined(); // General validation error
      }
      console.log('✅ Test: Create transaction with non-existent contactId failed as expected.');
    });

    test('Should fail to create transaction with missing transactionName', async () => {
      const transactionData = {
        contactId: userB ? userB.uid : 'dummy-contact-id-for-test', // Assume userB.uid would be a valid contact
        transactionType: 'simple_transfer',
        // transactionName is missing
        transactionAmount: '0.01',
        currency: 'ETH',
      };
      const response = await apiClient.post('/transaction/create', transactionData);
      expect(response.status).toBe(400);
      expect(response.body.error).toContain('Transaction name is required');
      console.log('✅ Test: Create transaction with missing name failed as expected.');
    });
    
    test('Should fail to create transaction with invalid amount (e.g., negative or non-numeric)', async () => {
      const transactionData = {
        contactId: userB ? userB.uid : 'dummy-contact-id-for-test',
        transactionType: 'simple_transfer',
        transactionName: 'Invalid Amount Test',
        transactionAmount: '-100', // Invalid
        currency: 'ETH',
      };
      const response = await apiClient.post('/transaction/create', transactionData);
      expect(response.status).toBe(400);
      // The specific error message will depend on your validation logic (e.g., Joi, custom)
      expect(response.body.error).toMatch(/Invalid transaction amount|Amount must be a positive number/i);
    ...
    test('Should fail to get non-existent transaction details', async () => {
      ...
      // This should be 404, but the basicFlow test checks for [404, 400]
      expect([404, 400]).toContain(response.status); 
      console.log('✅ Test: Get non-existent transaction failed as expected.');
    });

    // Add more tests for other transaction endpoints: /list, /sync-status, etc.
    ...
    test('Should fail to update non-existent transaction status', async () => {
        const response = await apiClient.put('/transaction/non-existent-tx-id/sync-status', { status: 'something' });
        // This might be a 404 if the transaction ID is checked first, or 400 if payload is validated first.
        // Given the basicFlow test for sync-status expected 400 on bad payload, let's assume ID check leads to 404.
        expect([404, 400]).toContain(response.status); 
        console.log('✅ Test: Update non-existent transaction status failed as expected.');
    });
  });

  // TODO: Add Negative Flows for File Operations if applicable
  ...
  //     expect(response.status).toBe(401);
  //     console.log('✅ Test: List files unauthenticated failed as expected.');
  //   });
  // });
}); 