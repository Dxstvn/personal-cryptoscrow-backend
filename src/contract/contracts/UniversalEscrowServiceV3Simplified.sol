...
/**
 * @title UniversalEscrowServiceV3Simplified
 * @notice Simplified escrow service using only Stargate for cross-chain transfers
 * @dev Removes redundant LayerZero OFT logic in favor of Stargate-only approach
 */
contract UniversalEscrowServiceV3Simplified is Ownable, ReentrancyGuard {
     ...
     */
    function updateCondition(bytes32 escrowId, bool conditionMet) external virtual onlyAuthorizedUpdater {
    ...
    function _getUSDCAddress(uint256 chainId) internal pure returns (address) {
        if (chainId == 11155111) return 0x2F6F07CDcf3588944Bf4C42aC74ff24bF56e7590; // Sepolia
        if (chainId == 421614) return 0x3253a335E7bFfB4790Aa4C25C4250d206E9b9773; // Arbitrum Sepolia
        return address(0);
    }
