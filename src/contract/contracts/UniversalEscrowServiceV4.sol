...
/**
 * @title UniversalEscrowServiceV4
 * @notice Enhanced escrow service with proper fee handling for LayerZero
 * @dev Fixes quote mismatch by using caller-provided fee directly
 */
contract UniversalEscrowServiceV4 is Ownable, ReentrancyGuard {
    ...
    function _handleCrossChainRelease(bytes32 escrowId, EscrowDeposit memory escrow, uint32 targetEndpointId) internal {
        ...
        // Use the fee provided by the caller
        // The caller should have already called quoteSend to get the correct fee
        require(msg.value > 0, "No fee provided");
        
        // Create fee structure from msg.value
        MessagingFee memory fee = MessagingFee({
            nativeFee: msg.value,
            lzTokenFee: 0
        });
        
        // Execute cross-chain transfer with the full msg.value
        (MessagingReceipt memory receipt, ) = IOFT(oftAdapter).send{value: msg.value}(
            sendParam,
            fee,
