...
/**
 * @title UniversalEscrowService
 * @notice Complete escrow service with 2% fee and intelligent routing
 * @dev Handles same-chain (Uniswap), cross-chain (LayerZero), and direct transfers
 */
contract UniversalEscrowService is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using OptionsBuilder for bytes;
    
    // Service fee (2% = 200 basis points)
    uint256 public constant SERVICE_FEE_BPS = 200;
    ...
    // LayerZero OFT adapters per chain
    mapping(uint32 => address) public oftAdapters; // chainId => adapter address
    mapping(uint32 => string) public chainNames; // chainId => readable name
    
    // Swap composers per chain for automatic token swaps
    mapping(uint32 => address) public swapComposers; // chainId => composer address
    
    // Gas limits for compose operations
    uint128 public lzReceiveGas = 100000; // Gas for receiving tokens
    uint128 public lzComposeGas = 500000; // Gas for swap execution
    
    // Escrow storage
    struct EscrowDeposit {
        address buyer;
        address seller;
        address depositToken; // Address(0) for ETH
        uint256 depositAmount;
        uint256 netAmount; // After service fee
        address targetToken; // What seller wants to receive
        uint32 targetChainId; // 0 for same chain
        bool released;
        bool conditionMet;
        uint256 timestamp;
    ...
    mapping(address => bytes32[]) public userEscrows; // buyer => escrow IDs
    ...
    event CrossChainTransferInitiated(
        ...
        address indexed oftAdapter,
        bytes32 layerZeroGuid,
        bool withCompose
    );
    ...
    // Authorized addresses that can update conditions (oracle/backend)
    mapping(address => bool) public conditionUpdaters;
    
    modifier onlyConditionUpdater() {
     ...
     * @param targetChainId Chain where seller wants to receive (0 for same chain)
    ...
    function createEscrow(
        ...
        if (seller == address(0) || depositAmount == 0) revert InvalidAmount();
     ...
     * @param met Whether the condition has been met
     */
    function updateCondition(bytes32 escrowId, bool met) external onlyConditionUpdater {
        ...
        emit ConditionUpdated(escrowId, met, msg.sender);
    }
    ...
    function _handleCrossChainRelease(bytes32 escrowId, EscrowDeposit memory escrow) internal {
        address oftAdapter = oftAdapters[escrow.targetChainId];
        ...
        address composer = swapComposers[escrow.targetChainId];
     ...
     */
    function setOFTAdapter(uint32 chainId, address adapter, string memory chainName) external onlyOwner {
        oftAdapters[chainId] = adapter;
     ...
     * @param composer Composer contract address on that chain
     */
    function setSwapComposer(uint32 chainId, address composer) external onlyOwner {
        swapComposers[chainId] = composer;
    }
    
    /**
     * @notice Update gas limits for LayerZero operations
     * @param _lzReceiveGas Gas for receiving tokens
     * @param _lzComposeGas Gas for compose operations
     */
    function setGasLimits(uint128 _lzReceiveGas, uint128 _lzComposeGas) external onlyOwner {
        lzReceiveGas = _lzReceiveGas;
        lzComposeGas = _lzComposeGas;
    }
    
    /**
     * @notice Get swap composer for a chain
     */
    function getSwapComposer(uint32 chainId) external view returns (address) {
        return swapComposers[chainId];
    }
