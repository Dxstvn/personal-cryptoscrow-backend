...
async function main() {
  ...
  // Test 3: ETH → WETH (direct conversion)
  console.log("\n🧪 TEST 3: ETH → WETH");
  await testSwap(escrow, user2, hre.ethers.ZeroAddress, weth.target, "0.1", weth, "ETH", "WETH");
  
  // Test 4: Token to Token (need to fund first)
  console.log("\n🧪 TEST 4: USDC → DAI");
...
async function testSwap(escrow, seller, fromToken, toToken, amountStr, toTokenContract, fromSymbol, toSymbol, fromDecimals = 18) {
  ...
  // Get initial balance
  const initialBalance = await toTokenContract.balanceOf(seller.address);
  ...
  const finalBalance = await toTokenContract.balanceOf(seller.address);
